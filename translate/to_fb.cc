...
main(int argc, char *argv[])
{
    ...
    struct mtop op;
#endif /* ifdef HAVE_SYS_MTIO_H */
    char * null_name = (char *)"/dev/null", *aa, *source_dev, *out_dev;

    buf = (char *)malloc (BLOCK_SIZE);
    buf[0] = 0;
    iptr = (int *)buf;
    *iptr = 1;
    hostIsLittleEndian() = buf[0] == 1 ? 1 : 0;
...
# endif
	...
	if( null_dev || dd_output || weof_count ) {
	  ...
	  /* don't write more tha one consecutive EOF */
	   ...
	   n = fb_write(out,buf,(int32_t)0); /* eof on disk */
	}
#ifdef HAVE_SYS_MTIO_H
	else {
	   weof_count++;
...
fb_write (int fout, char *buf, int count)
{
     ...
     */
    int32_t int size_rec=count, rlen1, rlen2=0, blip;

    if(hostIsLittleEndian()) {
//       swack4(&size_rec, &blip);  //Jul 26, 2011
       swack4((char *)&size_rec, (char *)&blip);  //Jul 26, 2011
    }
    else {
       blip = size_rec;
    }
