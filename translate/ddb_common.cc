...
{ (char *)"CAL", (char *)"PPI", (char *)"COP", (char *)"RHI", (char *)"VER",
  (char *)"TAR", (char *)"MAN", (char *)"IDL", (char *)"SUR", (char *)"AIR",
...
int 
dd_get_scan_mode (const char *mne)
{
    int jj;
...
struct cfac_wrap *
dd_absorb_cfacs (const char *list, const char *radar_name)
{
...
void 
dd_absorb_cfac (const char *list, const char *radar_name,
		struct correction_d *cfac)
{
    char *a;
    char str[256];
...
double 
dd_altitude (struct dd_general_info *dgi)
{
...
double 
dd_altitude_agl (struct dd_general_info *dgi)
{
...
void 
dd_alloc_data_field (DGI_PTR dgi, int pn)
{
    int mm;
    DDS_PTR dds = dgi->dds;
...
double 
dd_azimuth_angle (struct dd_general_info *dgi)
{
    double d_rot;
    ...
    else if(dgi->dds->radd->radar_type == DD_RADAR_TYPE_AIR_LF ||
...
int 
dd_cell_num (struct dds_structs *dds, double range)
{
    /* find the cell corresponding to this range using the
     * uniform spacing lookup table
     */
    int ii, *lut, ncMax = dds->celv->number_cells -1;
    float r;

    lut = dds->uniform_cell_lut[0];
    ii = (int)(.5 + (range - dds->uniform_cell_zero[0]) *
	  dds->rcp_uniform_cell_spacing[0]);
    if(ii < 0)
	  ii = 0;
    else if(ii >= dds->uniform_cell_count[0])
	  ii = dds->uniform_cell_count[0] -1;
...
void 
dd_clear_pisp (struct point_in_space *pisp)
{
    int nn=sizeof (*pisp);
    memset(pisp, 0, nn);
...
dd_clip_gate (
    DGI_PTR dgi,
    double elev,			/* assume elev in radians */
    double alt,			/* assume alt in km */
    double lower,
    double upper
)
{
...
int 
dd_compress (unsigned short *src, unsigned short *dst, int flag, int n)
{
...
void 
dd_copy_pisp (struct point_in_space *p0, struct point_in_space *p1)
{
...
int 
dd_datum_size (int binary_format)
{
...
double 
dd_drift (struct dd_general_info *dgi)
{
...
double 
dd_elevation_angle (struct dd_general_info *dgi)
{
    int mark;
    double d, d_rot;
...
char *
dd_find_desc (char *a, char *b, char *desc)
{
...
int 
dd_find_field (struct dd_general_info *dgi, const char *name)
{
    int ii=0, nc, nx;
...
void 
dd_get_lims (char *str_ptrs[], int ii, int nt, struct d_limits **dlims)
{
...
int 
dd_givfld (DGI_PTR dgi, int ndx, int g1, int n, float *dd, float *badval)
{
    /*
     ...
     *
     * returns number of gates copied
     */
    DDS_PTR dds=dgi->dds;
...
double 
dd_heading (struct dd_general_info *dgi)
{
...
dd_ini (const int rn, const char *radar_name)
{
    /*
	...
	sizeof_dgi = (int)((float)isize*1.01);
    }   

    // Initialize the structure

    dgi = new dd_general_info;
    dgi->time0 = 0.0;
    dgi->time = 0.0;
    dgi->vol_time0 = 0.0;
    dgi->prev_rot_angle = 0.0;
    dgi->start_time = 0;
    dgi->stop_time = 0;
    dgi->timeof_first_sweep = 0;
    dgi->timeof_last_sweep = 0;
    dgi->timeof_next_sweep = 0;
    dgi->volume_time_stamp = 0;
    dgi->sweep_time_stamp = 0;
    dgi->beam_count = 0;
    dgi->buf_bytes_left = 0;
    dgi->clip_gate = 0;
    dgi->compression_scheme = 0;
    dgi->disk_output = 0;
    dgi->file_byte_count = 0;
    dgi->ignore_cfacs = 0;
    dgi->ignore_this_sweep = 0;
    dgi->in_swp_fid = 0;
    dgi->source_ray_num = 0;
    dgi->last_sweep = 0;
    dgi->source_num_parms = 0;
    dgi->max_rat_entries = 0;
    dgi->nav_system = 0;
    dgi->new_sweep = 0;
    dgi->new_vol = 0;
    ...
    dgi->num_parms = 0;
    dgi->num_rdats = 0;
    dgi->offset_to_swib = 0;
    for (int i = 0; i < MAX_PARMS; ++i)
      dgi->parm_type[i] = 0;
    dgi->radar_num = rn;
    dgi->ray_num = 0;
    dgi->ray_quality = 0;
    dgi->sizeof_comments = 0;
    dgi->sizeof_dd_buf = 0;
    dgi->sizeof_dgi = sizeof_dgi;
    dgi->sizeof_seds = 0;
    dgi->source_fmt = 0;
    dgi->source_sweep_num = 0;
    dgi->source_vol_num = 0;
    dgi->sweep_count = 0;
    dgi->sweep_fid = 0;
    dgi->sweep_num = 0;
    ...
    dgi->gpptr6 = 0;
    dgi->gpptr7 = 0;
    dgi->prev_scan_mode = -1;
    dgi->prev_vol_num = -1;
    
    dgi->radar_name = radar_name;
    if(a=get_tagged_string("DORADE_DIR"))
    {
      dgi->directory_name = a;
      if (dgi->directory_name[dgi->directory_name.size()-1] != '/')
	dgi->directory_name += "/";
    }
    ...
    /* radar descriptor */
    radd  = dds->radd = dd_malloc_radd(dgi, (char *)"");
...
# endif
	 ...
	 */
	put_tagged_string((char *)"A_SPECKLE", a);
    }
    if(a=getenv("AC_NETCDF_ALIASES")) {
	put_tagged_string((char *)"AC_NETCDF_ALIASES", a);
    }
    if(a=getenv("AC_NETCDF_FILES")) {
	put_tagged_string((char *)"AC_NETCDF_FILES", a);
    }
    if(a=getenv("AC_TIME_CORRECTION")) {
	put_tagged_string((char *)"AC_TIME_CORRECTION", a);
    }
    if(a=getenv("ADS_START_DATE")) {
	put_tagged_string((char *)"ADS_START_DATE", a);
    }
    if(a=getenv("ADS_IO_TYPE")) {
	put_tagged_string((char *)"ADS_IO_TYPE", a);
    }
    if(a=getenv("ADS_TAPES")) {
	/* Electra raw data tapes */
	put_tagged_string((char *)"ADS_TAPES", a);
    }
    if(a=getenv("AFT_ANGLE_LIMITS")) {
	put_tagged_string((char *)"AFT_ANGLE_LIMITS", a);
    }
    if(a=getenv("AFT_REFL_CORR")) {
	put_tagged_string((char *)"AFT_REFL_CORR", a);
    }
    if(a=getenv("ALTITUDE_LIMITS")) {
	/* upper and lower limits for altitude clipping */
	put_tagged_string((char *)"ALTITUDE_LIMITS", a);
    }
    if(a=getenv("ASCENDING_ONLY")) {
	/* rejects non-ascending fixed angles */
	put_tagged_string((char *)"ASCENDING_ONLY", a);
    }
    if(a=getenv("AZ_SECTORS")) {
	put_tagged_string((char *)"AZ_SECTORS", a);
    }
    if(a=getenv("BATCH_MODE")) {
	put_tagged_string((char *)"BATCH_MODE", a);
    }
    if(a=getenv("BEAM_COUNT")) {
	/* stop after reading this number of beams */
	put_tagged_string((char *)"BEAM_COUNT", a);
    }
    if(a=getenv("BEAM_SKIP")) {
	/* only for ELDORA */
	put_tagged_string((char *)"BEAM_SKIP", a);
    }
    if(a=getenv("BIAS")) {
	put_tagged_string((char *)"BIAS", a);
    }
    if(a=getenv("CAPPI_HEIGHT")) {
	/* altitude msl in meters of the cappi plane */
	put_tagged_string((char *)"CAPPI_HEIGHT",a);
    }
    if(a=getenv("CAPPI_LATITUDES")) {
	put_tagged_string((char *)"CAPPI_LATITUDES", a);
    }
    if(a=getenv("CAPPI_LEVELS")) {
	put_tagged_string((char *)"CAPPI_LEVELS", a);
    }
    if(a=getenv("CAPPI_LONGITUDES")) {
	put_tagged_string((char *)"CAPPI_LONGITUDES", a);
    }
    if(a=getenv("CAPPI_RADAR")) {
	put_tagged_string((char *)"CAPPI_RADAR", a);
    }
    if(a=getenv("CAPPI_TIMES")) {
	put_tagged_string((char *)"CAPPI_TIMES", a);
    }
    if(a=getenv("CAPPI_RANGE_RES")) { /* resolution in meters */
	put_tagged_string((char *)"CAPPI_RANGE_RES",a);
    }
    if(a=getenv("CAPPI_REFL")) { /* source field name */
	put_tagged_string((char *)"CAPPI_REFL",a);
    }
    if(a=getenv("CAPPI_RESOLUTION")) {
	put_tagged_string((char *)"CAPPI_RESOLUTION", a);
    }
    if(a=getenv("CAPPI_VEL")) {
	put_tagged_string((char *)"CAPPI_VEL",a);
    }
    if(a=getenv("CAPPI_X_AXIS")) {
	put_tagged_string((char *)"CAPPI_X_AXIS", a);
    }
    if(a=getenv("CAPPI_Y_AXIS")) {
	put_tagged_string((char *)"CAPPI_Y_AXIS", a);
    }
	 ...
	 */
	put_tagged_string((char *)"CATALOG_FLUSH_COUNT",a);
    }
	 ...
	 */
	put_tagged_string((char *)"FIXED_ANGLES", a);
    }
    if(a=getenv("FLIGHT_ID")) {
	put_tagged_string((char *)"FLIGHT_ID", a);
    }
    if(a=getenv("FOF_OUTPUT_FIELDS")) {
	put_tagged_string((char *)"FOF_OUTPUT_FIELDS", a);
    }
    if(a=getenv("FOF_OMIT_TRANSITIONS")) {
	put_tagged_string((char *)"FOF_OMIT_TRANSITIONS", a);
    }
    if(a=getenv("FOLD_SHEAR")) {
	/* triggers an unfold if the shear is greater
	 * than this number
	 */
	put_tagged_string((char *)"FOLD_SHEAR", a);
    }
    if(a=getenv("FORE_ANGLE_LIMITS")) {
	put_tagged_string((char *)"FORE_ANGLE_LIMITS", a);
    }
    if(a=getenv("FORE_REFL_CORR")) {
	put_tagged_string((char *)"FORE_REFL_CORR", a);
    }
    if(a=getenv("GECHO_MAX_ROT_ANGLE")) { /* in degrees */
	put_tagged_string((char *)"GECHO_MAX_ROT_ANGLE", a);
    }
    if(a=getenv("GECHO_MIN_GATES")) {
	/* the ground echo program should always produce
	 * at least this number of gates per beam
	 */
	put_tagged_string((char *)"GECHO_MIN_GATES", a);
    }
    if(a=getenv("GECHO_MIN_ROT_ANGLE")) {
	put_tagged_string((char *)"GECHO_MIN_ROT_ANGLE", a);
    }
    if(a=getenv("GECHO_MSL")) {
	put_tagged_string((char *)"GECHO_MSL", a);
    }
    if(a=getenv("GECHO_REFL")) {
	put_tagged_string((char *)"GECHO_REFL",a);
    }
    if(a=getenv("GECHO_VEL")) {
	put_tagged_string((char *)"GECHO_VEL",a);
    }
    if(a=getenv("GENERATE_SUBSECOND_TIMES")) {
	put_tagged_string((char *)"GENERATE_SUBSECOND_TIMES", a);
    }
    if(a=getenv("GENERATE_THRESHOLDED_FIELDS")) {
	put_tagged_string((char *)"GENERATE_THRESHOLDED_FIELDS",a);
    }
    if(a=getenv("GPRO_TAPES")) {
	put_tagged_string((char *)"GPRO_TAPES", a);
    }
    if(a=getenv("HRD_ASCII_TAPE")) {
	put_tagged_string((char *)"HRD_ASCII_TAPE", a);
    }
    if(a=getenv("HRD_IO_TYPE")) {
	put_tagged_string((char *)"HRD_IO_TYPE", a);
    }
    if(a=getenv("HRD_RANGE_DELAY")) {
	/* the number of gates to throw away
	 * at the beginning of the beam
	 */
	put_tagged_string((char *)"HRD_RANGE_DELAY",a);
    }
    if(a=getenv("HRD_STD_TAPE")) {
	put_tagged_string((char *)"HRD_STD_TAPE", a);
    }
    if(a=getenv("HRD_VOLUME_HEADER")) {
	/* specifies the complete file name of a surrogate
	 * volume header record
	 */
	put_tagged_string((char *)"HRD_VOLUME_HEADER",a);
    }
    if(a=getenv("IMPROVE_HZO_RCONST_CORR")) {
	put_tagged_string((char *)"IMPROVE_HZO_RCONST_CORR", a);
    }
    if(a=getenv("INCLUDE_DH_DP")) {
	put_tagged_string((char *)"INCLUDE_DH_DP", a);
    }
    if(a=getenv("INPUT_FORMAT")) {
	put_tagged_string((char *)"INPUT_FORMAT", a);
    }
    if(a=getenv("INPUT_FILES_LIST")) {
	put_tagged_string((char *)"INPUT_FILES_LIST", a);
    }
    if(a=getenv("INTERLEAVE")) {
	put_tagged_string((char *)"INTERLEAVE", a);
    }
    if(a=getenv("IO_TYPE")) {	/* see DORADE_IO_TYPE */
	put_tagged_string((char *)"IO_TYPE", a);
    }
    if(a=getenv("JULIAN_DAY_BIAS")) {
	/* the number of days that will be subtracted from the
	 * julian day in the ray info descriptor
	 */
	put_tagged_string((char *)"JULIAN_DAY_BIAS", a);
    }
    if(a=getenv("KEEP_ORTHOG0NAL_DATA")) {
	put_tagged_string((char *)"KEEP_ORTHOG0NAL_DATA", a);
    }
    if(a=getenv("LIDAR_AVERAGING")) {
	put_tagged_string((char *)"LIDAR_AVERAGING", a);
    }
    if(a=getenv("LIDAR_CELL_SPACING")) {
	put_tagged_string((char *)"LIDAR_CELL_SPACING", a);
    }
    if(a=getenv("LIDAR_OPTIONS")) {
	put_tagged_string((char *)"LIDAR_OPTIONS", a);
    }
    if(a=getenv("LIDAR_SWEEP_TIME_LIMIT")) {
	put_tagged_string((char *)"LIDAR_SWEEP_TIME_LIMIT", a);
    }
    if(a=getenv("LINES_IN_WINDOW")) {
	put_tagged_string((char *)"LINES_IN_WINDOW", a);
    }
    if(a=getenv("MAX_DORADE_TAPE_SIZE")) {
	put_tagged_string((char *)"MAX_DORADE_TAPE_SIZE", a); /* in GB */
    }
    if((a=getenv("MAX_MEDIA_SIZE"))) {
	/* in MB */
	put_tagged_string((char *)"MAX_MEDIA_SIZE", a);
    }
    if(a=getenv("MAX_NOTCH_VELOCITY")) {
	/* The maximum (in m/s) that a notch velocity
	 * can attain
	 */
	put_tagged_string((char *)"MAX_NOTCH_VELOCITY", a);
    }
    if(a=getenv("MAX_RAYS_PER_SWEEP")) {
	put_tagged_string((char *)"MAX_RAYS_PER_SWEEP", a);
    }
    if(a=getenv("MAX_RHI_DIFF")) {
	put_tagged_string((char *)"MAX_RHI_DIFF", a);
    }
    if(a=getenv("MAX_SWEEPS_PER_VOLUME")) {
	put_tagged_string((char *)"MAX_SWEEPS_PER_VOLUME", a);
    }
    if(a=getenv("MAX_UF_CELLS")) {
	/* fixes the number of uf range bins at some max value */
	put_tagged_string((char *)"MAX_UF_CELLS", a);
    }
    if(a=getenv("MAX_VOLUME_SIZE")) {
	/* in MagaBytes
	 * DORADE volume must only slightly exceed this size
	 */
	put_tagged_string((char *)"MAX_VOLUME_SIZE", a);
    }
    if(a=getenv("MAX_WIND")) {
	put_tagged_string((char *)"MAX_WIND", a);
    }
    if(a=getenv("MAX_AZ_DIFF")) {
	put_tagged_string((char *)"MAX_AZ_DIFF", a);
    }
    if(a=getenv("MC_DATA")) {
	put_tagged_string((char *)"MC_DATA", a);
    }
    if(a=getenv("MIN_AZ_DIFF")) {
	put_tagged_string((char *)"MIN_AZ_DIFF", a);
    }
    if(a=getenv("MIN_BAD_COUNT")) {
	/* In some cases is used to stop an algorithm
	 * after more than n consecutive bad cells
	 */
	put_tagged_string((char *)"MIN_BAD_COUNT", a);
    }
    if(a=getenv("MIN_EL_DIFF")) {
	put_tagged_string((char *)"MIN_EL_DIFF", a);
    }
    if(a=getenv("MIN_FXD_DIFF")) {
	put_tagged_string((char *)"MIN_FXD_DIFF", a);
    }
    if(a=getenv("MIN_FREE_MB")) {
	/* stops creating new sweep files when free space on disk
	 * drops below this value
	 */
	put_tagged_string((char *)"MIN_FREE_MB", a);
    }
    if(a=getenv("MIN_RAYS_PER_SWEEP")) {
	put_tagged_string((char *)"MIN_RAYS_PER_SWEEP", a);
    }
    if(a=getenv("MIN_SWEEPS_PER_VOLUME")) {
	put_tagged_string((char *)"MIN_SWEEPS_PER_VOLUME", a);
    }
    if(a=getenv("MIN_TIME_GAP")) {
	/* In some cases is used to start a new volume
	 * after more than n seconds of missing data
	 */
	put_tagged_string((char *)"MIN_TIME_GAP", a);
    }
    if(a=getenv("MIN_VOLUME_TIME_SPAN")) {
	/* in seconds
	 * DORADE volume must be a least this long
	 */
	put_tagged_string((char *)"MIN_VOLUME_TIME_SPAN", a);
    }
    if(a=getenv("MRD_DZ_FIELD")) {
	put_tagged_string((char *)"MRD_DZ_FIELD", a);
    }
    if(a=getenv("MRD_MAX_RANGE")) {
	put_tagged_string((char *)"MRD_MAX_RANGE", a);
    }
    if(a=getenv("MRD_VE_FIELD")) {
	put_tagged_string((char *)"MRD_VE_FIELD", a);
    }
    if((a=getenv("NETCDF_DIRECTORY")) || (a=getenv("NC_DIR"))) {
	put_tagged_string((char *)"NC_DIR", a);
    }
    if(a=getenv("NETCDF_FIELDS")) {
	put_tagged_string((char *)"NETCDF_FIELDS", a);
    }
    if(a=getenv("NEW_SWEEP_FLAGS")) {
	put_tagged_string((char *)"NEW_SWEEP_FLAGS", a);
    }
    if(a=getenv("NOTCH_SHEAR")) {
	/* triggers denotching if the shear is greater
	 * than this number
	 */
	put_tagged_string((char *)"NOTCH_SHEAR", a);
    }
    if(a=getenv("NCP_THRESHOLD_VAL")) {
	put_tagged_string((char *)"NCP_THRESHOLD_VAL", a);
    }
    if(a=getenv("NEW_SWEEP_HESITATION")) {
	put_tagged_string((char *)"NEW_SWEEP_HESITATION", a);
    }
    if(a=getenv("NO_WIND_INFO")) {
	/* do not use wind info in unfolding */
	put_tagged_string((char *)"NO_WIND_INFO", a);
    }
    if(a=getenv("NUM_AZ_DIFFS_AVGD")) {
	put_tagged_string((char *)"NUM_AZ_DIFFS_AVGD", a);
    }
    if(a=getenv("NUM_EL_DIFFS_AVGD")) {
	put_tagged_string((char *)"NUM_EL_DIFFS_AVGD", a);
    }
    if(a=getenv("NUM_SHORT_AVG")) {
	put_tagged_string((char *)"NUM_SHORT_AVG", a);
    }
    if(a=getenv("NYQUIST_VELOCITY")) {
	put_tagged_string((char *)"NYQUIST_VELOCITY", a);
    }
    if(a=getenv("OPTIONS")) {
	put_tagged_string((char *)"OPTIONS", a);
    }
    if(a=getenv("OUTPUT_FIELDS")) {
	put_tagged_string((char *)"OUTPUT_FIELDS", a);
    }
	 ...
	 * "GECHO_DATA"
	 */
	put_tagged_string((char *)"OUTPUT_FLAGS", a);
    }
    if(a=getenv("PCT_STATS")) {
	put_tagged_string((char *)"PCT_STATS", a);
    }
    if(a=getenv("PHASE_OFFSET")) {
	put_tagged_string((char *)"PHASE_OFFSET", a);
    }
    if(a=getenv("PPI_EL_TOL")) {
	put_tagged_string((char *)"PPI_EL_TOL", a);
    }
    if(a=getenv("RHI_AZ_TOL")) {
	put_tagged_string((char *)"RHI_AZ_TOL", a);
    }
    if(a=getenv("ZDR_OFFSET")) {
	put_tagged_string((char *)"ZDR_OFFSET", a);
    }
    if(a=getenv("PIRAQ_FORCE_POLYPP")) {
	put_tagged_string((char *)"PIRAQ_FORCE_POLYPP", a);
    }
    if(a=getenv("PRESERVE_SWEEP_FILES")) {
	/* prevents DORADE tape output routine from
	 * deleting used up sweep files
	 */
	put_tagged_string((char *)"PRESERVE_SWEEP_FILES", a);
    }
    if(a=getenv("PRF_LIMITS")) { /* see fixed angle */
	put_tagged_string((char *)"PRF_LIMITS", a);
    }
    if(a=getenv("PROJECT_NAME")) {
	put_tagged_string((char *)"PROJECT_NAME", a);
    }
    if(a=getenv("PWR_THRESHOLD_VAL")) {
	put_tagged_string((char *)"PWR_THRESHOLD_VAL", a);
    }
    if(a=getenv("RADAR_ALTITUDE")) {
	put_tagged_string((char *)"RADAR_ALTITUDE", a);
    }
    if(a=getenv("RCONST_CORRECTION")) {
	put_tagged_string((char *)"RCONST_CORRECTION", a);
    }
    if(a=getenv("RADAR_LATITUDE")) {
	put_tagged_string((char *)"RADAR_LATITUDE", a);
    }
    if(a=getenv("RADAR_LONGITUDE")) {
	put_tagged_string((char *)"RADAR_LONGITUDE", a);
    }
    if(a=getenv("RADAR_WAVELENGTH")) {
	put_tagged_string((char *)"RADAR_WAVELENGTH", a);
    }
    if(a=getenv("RANGE_CORRECTION")) {
	put_tagged_string((char *)"RANGE_CORRECTION", a);
    }
    if(a=getenv("RANGE_DELAY")) {
	/* the number of gates to throw away
	 * at the beginning of the beam
	 */
	put_tagged_string((char *)"RANGE_DELAY",a);
    }
    if(a=getenv("RADAR_NAME")) {
	put_tagged_string((char *)"RADAR_NAME", a);
    }
    if(a=getenv("RANGE_LIMITS")) {
	put_tagged_string((char *)"RANGE_LIMITS", a);
    }
    if(a=getenv("RENAME")) {
	put_tagged_string((char *)"RENAME", a);
    }
    if(a=getenv("ROTATION_ANGLE_TOLERANCE")) {
	put_tagged_string((char *)"ROTATION_ANGLE_TOLERANCE", a);
    }
    if(a=getenv("SCAN_LIST")) {
	put_tagged_string((char *)"SCAN_LIST", a);
    }
    if((a=getenv("SELECT_RADAR")) || (a=getenv("SELECT_RADARS"))) {
	put_tagged_string((char *)"SELECT_RADARS", a);
    }
    if(a=getenv("SHANES_DATA_FIELDS")) {
	put_tagged_string((char *)"SHANES_DATA_FIELDS", a);
    }
    if(a=getenv("SHANES_DIRECTORY")) {
	put_tagged_string((char *)"SHANES_DIRECTORY", a);
    }
    if(a=getenv("SITE_NAME")) {
	put_tagged_string((char *)"SITE_NAME", a);
    }
    if(a=getenv("SIDELOBE_RING")) {
	put_tagged_string((char *)"SIDELOBE_RING", a);
    }
    if(a=getenv("SNR_THRESHOLD_VAL")) {
	put_tagged_string((char *)"SNR_THRESHOLD_VAL", a);
    }
    if(a=getenv("SOURCE_DEV")) {
	/* the device or file name
	 */
	put_tagged_string((char *)"SOURCE_DEV",a);
    }
    if(a=getenv("SOURCE_FILE")) {
	/* names the input file/device */
	put_tagged_string((char *)"SOURCE_FILE", a);
    }
    if(a=getenv("SOURCE_TAPE_ID")) {
	/* goes into the catalog as the source tape id */
	put_tagged_string((char *)"SOURCE_TAPE_ID", a);
    }
    if(a=getenv("START_RANGE")) {
	put_tagged_string((char *)"START_RANGE", a);
    }
    if(a=getenv("STOP_RANGE")) {
	put_tagged_string((char *)"STOP_RANGE", a);
    }
    if(a=getenv("STORM_NAME")) {
	put_tagged_string((char *)"STORM_NAME", a);
    }
    if(a=getenv("SW_THRESHOLD_VAL")) {
	put_tagged_string((char *)"SW_THRESHOLD_VAL", a);
    }
    if(a=getenv("SWEEP_COUNT")) {
	/* stops input after reading n sweeps */
	put_tagged_string((char *)"SWEEP_COUNT", a);
    }
    if((a=getenv("SWEEP_MODE")) || (a=getenv("SWEEP_MODES"))) {
	/* filters on sweep modes such as "PPI", "AIR", etc. */
	put_tagged_string((char *)"SWEEP_MODES", a);
    }
    if(a=getenv("SWEEP_SKIP")) {
	/* uses every nth sweep of a particular radar */
	put_tagged_string((char *)"SWEEP_SKIP", a);
    }
    if(a=getenv("SWEEP_TIME_TOLERANCE")) {
	put_tagged_string((char *)"SWEEP_TIME_TOLERANCE", a);
    }
    if(a=getenv("SWEEP_TRIP_ANGLE")) {
	put_tagged_string((char *)"SWEEP_TRIP_ANGLE", a);
    }
    if(a=getenv("SWEEP_TRIP_DELTA")) {
	put_tagged_string((char *)"SWEEP_TRIP_DELTA", a);
    }
    if(a=getenv("TAPE_DIR")) {
	put_tagged_string((char *)"TAPE_DIR", a);
    }
    if(a=getenv("TEST_PULSE_RANGES")) {
	put_tagged_string((char *)"TEST_PULSE_RANGES", a);
    }
    if(a=getenv("TIME_CORRECTION")) {
	put_tagged_string((char *)"TIME_CORRECTION", a);
    }
    if(a=getenv("TIME_DEFINED_VOLUMES")) {
	put_tagged_string((char *)"TIME_DEFINED_VOLUMES", a);
    }
    if(a=getenv("TIME_DEPENDENT_FIXES")) {
	put_tagged_string((char *)"TIME_DEPENDENT_FIXES", a);
    }
    if(a=getenv("TIME_LIMITS")) {
	/* one or more sets of limits of the form
	 * "02/04/93:22:14:13 < 02/04/93:22:21:18"
	 */
	put_tagged_string((char *)"TIME_LIMITS", a);
    }
    if(a=getenv("TIME_DRIFT")) {
	put_tagged_string((char *)"TIME_DRIFT", a);
    }
    if(a=getenv("TOLERANCE_LEVEL_1")) {
	put_tagged_string((char *)"TOLERANCE_LEVEL_1", a);
    }
    if(a=getenv("TOLERANCE_LEVEL_2")) {
	put_tagged_string((char *)"TOLERANCE_LEVEL_2", a);
    }
    if(a=getenv("UF_ALIASES")) {
	put_tagged_string((char *)"UF_ALIASES", a);
    }
    if((a=getenv("UF_DEV")) || (a=getenv("UF_DEVS"))) {
	/* the device name if not a standard named file
	 * to which UF tape format data is written
	 */
	put_tagged_string((char *)"UF_DEV",a);
    }
    if(a=getenv("UF_DIRECTORY")) {
	/* destination of uf files */
	put_tagged_string((char *)"UF_DIRECTORY", a);
    }
    if(a=getenv("UF_IO_TYPE")) { /* see dorade io type */
	put_tagged_string((char *)"UF_IO_TYPE",a);
    }
    if(a=getenv("UF_OUTPUT_FIELDS")) {
	put_tagged_string((char *)"UF_OUTPUT_FIELDS", a);
    }
	 ...
	 */
	put_tagged_string((char *)"UNFOLD_QUE_SIZE", a);
    }
    if(a=getenv("UNIFORM_CELL_SPACING")) {
	put_tagged_string((char *)"UNIFORM_CELL_SPACING", a);
    }
    if(a=getenv("VOLUME_COUNT")) {
	/* stops input after reading n volumes */
	put_tagged_string((char *)"VOLUME_COUNT", a);
    }
    if(a=getenv("VOLUME_HEADER")) {
	/* specifies the complete file name of a surrogate
	 * volume header record
	 */
	put_tagged_string((char *)"VOLUME_HEADER",a);
    }
    if(a=getenv("XOUT_AZIMUTHS")) {
	put_tagged_string((char *)"XOUT_AZIMUTHS", a);
    }
    if(a=getenv("XOUT_ELEVATIONS")) {
	put_tagged_string((char *)"XOUT_ELEVATIONS", a);
    }
    if(a=getenv("WSR_QC_FILE")) {
	put_tagged_string((char *)"WSR_QC_FILE", a);
    }
    if(a=getenv("WATCH_FIXED_ANGLE")) {
	put_tagged_string((char *)"WATCH_FIXED_ANGLE", a);
    }
    if(a=getenv("YEAR_OF_DATA")) {
	put_tagged_string((char *)"YEAR_OF_DATA", a);
    }
    if(a=getenv("ZDR_BIAS")) {
	put_tagged_string((char *)"ZDR_BIAS", a);
    }
...
void 
dgi_interest_really (struct dd_general_info *dgi, int verbosity, char *preamble, char *postamble, struct sweepinfo_d *swib)
{
    /* 
...
double 
dd_latitude (struct dd_general_info *dgi)
{
...
void 
dd_latlon_relative (struct point_in_space *p0, struct point_in_space *p1)
{
    ...
    double R_earth = dd_earthr(p1->latitude); 
...
int 
loop_ll2xy_v3 (double *plat, double *plon, double *palt, double *x, double *y, double *z, double olat, double olon, double oalt, double R_earth, int num_pts)
{
...
void 
dd_latlon_shift (struct point_in_space *p0, struct point_in_space *p1)
{
...
int 
loop_xy2ll_v3 (double *plat, double *plon, double *palt, double *x, double *y, double *z, double olat, double olon, double oalt, double R_earth, int num_pts)
{
...
char *
dd_link_dev_name (const char *radar_name, const char *dev_links,
		  char *str)
{
...
double 
dd_longitude (struct dd_general_info *dgi)
{
...
struct radar_d *
dd_malloc_radd (DGI_PTR dgi, char *config_name)
{
...
double 
dd_nav_rotation_angle (struct dd_general_info *dgi)
{
    double d_rot;
    ...
    case DD_SCAN_MODE_TAR:
	d_rot = dgi->dds->radd->radar_type != DD_RADAR_TYPE_GROUND ?
...
double 
dd_nav_tilt_angle (struct dd_general_info *dgi)
{
    double d_rot;
...
int 
dd_ndx_name (DGI_PTR dgi, char *name2)
{
    DDS_PTR dds=dgi->dds;
...
double 
dd_pitch (struct dd_general_info *dgi)
{
...
void 
dor_print_asib (struct platform_i *asib, struct solo_list_mgmt *slm)
{
    char *aa, bstr[128];
...
void 
dor_print_celv (struct cell_d *celv, struct solo_list_mgmt *slm)
{
    int ii;
...
void 
dor_print_cfac (struct correction_d *cfac, struct solo_list_mgmt *slm)
{
    char str[64];
    char *aa, bstr[128];
...
void 
dor_print_parm (struct parameter_d *parm, struct solo_list_mgmt *slm)
{
    char str[64];
    char *aa, bstr[128];
...
void 
dor_print_frib (struct field_radar_i *frib, struct solo_list_mgmt *slm)
{
...
void 
dor_print_radd (struct radar_d *radd, struct solo_list_mgmt *slm)
{
    char str[128];
    char *aa, bstr[128];
...
void 
dor_print_rktb (struct rot_ang_table *rktb, struct solo_list_mgmt *slm)
{
    char *aa, *bb, bstr[128];
    int ii;
...
void 
dor_print_ryib (struct ray_i *ryib, struct solo_list_mgmt *slm)
{
    char *aa, bstr[128];
...
void 
dor_print_swib (struct sweepinfo_d *swib, struct solo_list_mgmt *slm)
{
    char *aa, bstr[128];
...
void 
dor_print_sswb (struct super_SWIB *sswb, struct solo_list_mgmt *slm)
{
    char str[128];
    char *aa, bstr[1024];
    DD_TIME dts;
    ...
    for(int ii=0; ii < sswb->num_key_tables; ii++) {
...
void 
dor_print_vold (struct volume_d *vold, struct solo_list_mgmt *slm)
{
    char str[128];
    char *aa, bstr[128];
...
void 
dd_radar_angles (struct platform_i *asib, struct correction_d *cfac, struct radar_angles *ra, struct dd_general_info *dgi)
{
...
char *
dd_radar_name (struct dds_structs *dds)
{
    int i;
    static char name[12];
    char *a;
...
void 
dd_radar_selected (char *radar_name, int radar_num, struct dd_input_filters *difs)
{
    int ii, jj, kk, nt;
    char *a, *b, *c, str[256], radar[16];
    char *str_ptrs[32];
...
void 
dd_range_gate (DGI_PTR dgi, const float req_range, int *gate, float *val)
{
...
void 
dd_razel_to_xyz (struct point_in_space *p0)
{
...
void 
dd_reset_d_limits (struct d_limits **at)
{
    if(!(*at)) {
...
dd_return_rotang1 (struct rot_ang_table *rat)
{
...
double 
dd_roll (struct dd_general_info *dgi)
{
...
int dd_rotang_seek(struct rot_ang_table *rat, double rotang)
{
    ...
    ang_ndx = (int)(rotang*rat->angle2ndx);
...
void 
dd_rotang_table (DGI_PTR dgi, int func)
{
...
double 
dd_rotation_angle (struct dd_general_info *dgi)
{
...
void 
dd_rng_info (DGI_PTR dgi, int *g1, int *m, float *rngs, int *ng)
{
    DDS_PTR dds=dgi->dds;
...
void 
dd_set_uniform_cells (struct dds_structs *dds)
{
    ...
    if((nc = (int)(1.5 + r/cell_spacing)) > 32000) {
...
void 
dd_site_name (char *proj_name, char *site_name)
{
...
double 
dd_tilt_angle (struct dd_general_info *dgi)
{
    double d_rot;
...
int 
dd_tokens (char *att, char *str_ptrs[])
{
    int nargs=0;
    char *b=att, *dlims=(char *)" \t\n"; /* blank, tab, and new line */
...
dd_tokenz (
    char *att,
    char *str_ptrs[],
    const char *dlims /* token delimiters */
)
{
    int nargs=0;
    char *b=att;
...
void 
dd_uniform_cell_spacing (float src_cells[], int num_cells, double cell_spacing, int *cell_lut, double r0, int nuc)
{
...
int 
dd_xy_plane_horizontal (struct dd_general_info *dgi)
{
    int ii, nn;
    
    nn = dgi->dds->radd->radar_type == DD_SCAN_MODE_AIR ? NO : YES;
    return(nn);
}
/* c------------------------------------------------------------------------ */

int 
fp_bin_search (float *f, int nf, double val)
{
...
dgi_buf_rewind (DGI_PTR dgi)
{
	...
	if(!dgi->in_buf) {
	    printf("Unable to malloc dgi->in_buf for: %s %d\n",
		   dgi->radar_name.c_str(), dgi->radar_num);
	    exit(1);
	}
    ...
    if(mark = lseek(dgi->in_swp_fid, 0L, SEEK_SET)) {
	mark = 0;
    }
...
int 
HRD_recompress (unsigned short *src, unsigned short *dst, int flag, int n, int compression)
{
    /* implement hrd compression of 16-bit values
     * and return the number of 16-bit words of compressed data
     */
...
int 
RDP_compress_bytes (unsigned char *src, unsigned char *dst, int flag, int n, int compression)
{
...
int32_t 
xswap4 ( /* assemble an integer*4 variable */
    char *v_lower,
    char *v_upper
)
{
